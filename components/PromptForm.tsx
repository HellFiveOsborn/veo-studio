...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const [durationSeconds, setDurationSeconds] = useState(
    initialValues?.durationSeconds ?? 8,
  );
  ...
  const promptPlaceholder = {
    ...
    [GenerationMode.FRAMES_TO_VIDEO]:
      'Descreva o movimento entre os frames inicial e final (opcional)...',
    [GenerationMode.REFERENCES_TO_VIDEO]:
      'Descreva um vídeo usando imagens de referência e estilo...',
    [GenerationMode.EXTEND_VIDEO]:
      'Descreva o que acontece a seguir (opcional)...',
  }[generationMode];
          ...
          <VideoUpload
            label={
              <>
                Vídeo de Entrada
                <br />
                (deve ser gerado pelo Veo em 720p)
              </>
            }
            video={inputVideo}
  ...
  switch (generationMode) {
    case GenerationMode.TEXT_TO_VIDEO:
      ...
      if (isSubmitDisabled) {
        tooltipText = 'Por favor, insira um comando.';
      }
      break;
    case GenerationMode.FRAMES_TO_VIDEO:
      isSubmitDisabled = !startFrame;
      if (isSubmitDisabled) {
        tooltipText = 'Um frame inicial é necessário.';
      }
      break;
    case GenerationMode.REFERENCES_TO_VIDEO:
      ...
      if (hasNoRefs && hasNoPrompt) {
        tooltipText =
          'Por favor, adicione imagem(ns) de referência e insira um comando.';
      } else if (hasNoRefs) {
        tooltipText = 'É necessária pelo menos uma imagem de referência.';
      } else if (hasNoPrompt) {
        tooltipText = 'Por favor, insira um comando.';
      }
      break;
  ...
  return (
    <div className="relative w-full">
            ...
            <div>
              ...
              {isVeo2_0 && (
                <p className="text-xs text-gray-500 mt-2">
                  Resolução não é suportada por este modelo.
                </p>
              )}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[#1f1f1f] border border-gray-600 rounded-lg px-4 py-2.5 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                />
              </div>
              <p className="text-xs text-gray-500 mt-1">
                Use somente se precisar de um endpoint personalizado. Deixe em
      ...
      {!apiKey && (
        <div className="mb-3 p-4 bg-yellow-900/30 border border-yellow-600 rounded-xl flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <AlertTriangleIcon className="w-6 h-6 text-yellow-400 shrink-0" />
            <p className="text-yellow-300 text-sm">
              Sua chave de API do Gemini não está definida. É necessária para
              gerar vídeos.
            </p>
          </div>
          <button
            ...
            className="flex-shrink-0 px-4 py-2 bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors text-white font-semibold text-sm">
            Definir Chave
          </button>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="w-full">
        ...
        <p className="text-xs text-gray-500 text-center mt-2 px-4">
          O Veo é um modelo pago. Você será cobrado em seu projeto Cloud. Veja{' '}
          <a
